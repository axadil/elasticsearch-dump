...
elasticsearch.prototype.getMapping = function (limit, offset, callback){
    var self = this;
    var strictSSL = self.parent.options.strictSSL;
    if(self.gotMapping === true){
        callback(null, []);
    }else{
        var url = self.baseUrl + '/_mapping';
        request.get({uri:url,strictSSL:strictSSL}, function(err, response){
...
elasticsearch.prototype.numberOfShards = function(baseUrl, callback){
    self = this;
    var strictSSL = self.parent.options.strictSSL;
    request.get({uri:baseUrl + "/_settings",strictSSL:strictSSL}, function(err, response){
        if(err){
            callback(err);
        }else{
            try{
